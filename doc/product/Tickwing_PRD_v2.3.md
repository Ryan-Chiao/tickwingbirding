...
**文档维护人：** PM (Claude)
**语言策略：** 首发版本仅简体中文，架构预留 i18n（语言包独立），后续版本扩展英文
...
| v2.2 | 2026-03-08 | 落实所有 OQ 决策，新增：账号注销需求、视频规格约束（2min/H.265/2K/≤300MB）、坐标脱敏方案、i18n 策略、审核策略、视频删除流程、里程碑评审机制、云厂商策略（待定） | PM (Claude) |
| v2.3 | 2026-03-09 | 后端 Lead 评审反馈落实：新增 refresh_tokens 表、feeders.bind_code_expires_at 字段、notifications.archived_at 字段及清理索引、videos.recorded_at 条件索引；通知已读接口 PUT→PATCH；Phase 2 增加 1 周 buffer（4→5 周），后续 Phase 日期顺延；计数字段并发策略、鸟种字典表列为待决策项 | PM (Claude) |
...
本 PRD 是 Tickwing 项目的可执行需求基线，用于：
...
Tickwing 是一个连接观鸟爱好者、智能喂鸟硬件和生态数据的社区平台。用户既可以手动上传观鸟视频，也可以通过绑定喂鸟器自动采集视频，形成持续的内容流和社区互动。
...
1. 用户提交视频元数据并请求上传凭证
2. 服务端校验视频规格（时长 ≤ 2min，文件 ≤ 300MB，格式 MP4/MOV）
3. 客户端直传对象存储（分片 / 断点续传）
4. 服务端记录 `video = uploaded`
5. 异步转码队列触发 FFmpeg 任务
6. 转码成功后 `video = ready`，生成 HLS 与缩略图
7. 内容审核（见 4.5 审核策略）
...
1. 管理员创建公有设备并配置地理信息
2. 设备按 API Key 上传
3. 视频归属 `public_feeder_pool`（uploader_id 为系统账号 `system_feeder`）
4. 平台按规则分发到公共频道与推荐流
...
**首发版本采用"先发后审"策略：**
- 视频转码完成后立即可见（`moderation_status = auto_approved`）
- 用户举报或系统规则命中后进入人工审核队列
- 未来可升级为接入第三方内容安全服务（如阿里云内容安全 / AWS Rekognition）做自动预审

**审核结果分级：**
- `auto_approved` — 自动通过（未触发规则）
- `approved` — 人工审核通过
- `rejected` — 审核不通过，下架并通知用户
- `restricted` — 限流，不进入推荐流但可通过链接访问

### 4.6 视频删除流程

1. 用户或管理员发起删除
2. 视频标记为 `status = deleted`（软删除），立即从所有列表和搜索中移除
3. 关联的 comments / likes / bird_tags 随 CASCADE 删除
4. 异步 Worker 在 **7 天后**清理对象存储中的文件（raw + HLS + 缩略图）
5. 7 天内支持管理员恢复误删视频
...
| FR-AUTH-004 | 第三方登录 (Apple/Google) | P2 | 账号可与邮箱账号合并 (**决策：首发不含，后续迭代**) |
| FR-AUTH-005 | 风险登录告警 | P1 | 新设备登录触发通知 |
| FR-AUTH-006 | 账号注销与数据删除 | P1 | 注销后 30 天内完成用户数据清理，符合 GDPR / Apple App Store 要求 |
...
| ID | 需求 | 优先级 | 验收标准 |
|----|------|--------|---------|
| FR-VIDEO-001 | 手动上传 MP4/MOV (≤ 300MB) | P0 | 大文件支持断点续传。**视频规格：时长 ≤ 2 分钟，H.265 编码，最高 2K (2560×1440) 分辨率** |
...
| FR-FEEDER-003 | 心跳上报与在线状态 | P0 | 心跳超时自动离线，**默认超时阈值 5 分钟（可配置）** |
...
| NFR-SEC-006 | **坐标脱敏**：面向前端展示的经纬度精度降至 ±1km（约小数点后 2 位），原始精度仅后端存储供统计使用 |

### 6.5 国际化 (i18n)

- 首发版本仅支持**简体中文**
- 前端文案通过独立语言包管理（JSON / YAML），不硬编码到组件中
- 后端错误码和消息使用 code-based 方式，前端根据 locale 映射显示文案
- 后续版本扩展英文时，仅需新增语言包文件，无需修改业务代码
...
- **开发/测试阶段**：本地环境，使用 MinIO 模拟对象存储，Docker Compose 一键启动全部依赖
- **生产阶段**：云服务商待定（AWS 或阿里云，Phase 2 staging 上线前决策）
- 对象存储接口统一封装（Adapter 模式），底层可切换 S3 / OSS / MinIO
- 配置通过环境变量注入，不硬编码云厂商特定逻辑
- 架构设计确保单一云厂商即可运行，不要求双栈同时部署
...
CREATE TABLE refresh_tokens (
  ...
  device_info TEXT,                           -- User-Agent 等设备信息
...
CREATE TABLE feeders (
  ...
  latitude      DECIMAL(10, 8),  -- 原始精度
  longitude     DECIMAL(11, 8),  -- 原始精度
  display_lat   DECIMAL(10, 2),  -- 脱敏精度 (±1km)
  display_lng   DECIMAL(11, 2),  -- 脱敏精度 (±1km)
  is_online     BOOLEAN DEFAULT false,
  api_key       VARCHAR(255) UNIQUE NOT NULL,
  bind_code     VARCHAR(20),                    -- 一次性绑定码
  bind_code_expires_at TIMESTAMPTZ,              -- 绑定码过期时间（默认 15 分钟）
...
CREATE TABLE videos (
  ...
  status        VARCHAR(20) DEFAULT 'uploaded',  -- uploaded / queued / processing / ready / failed / hidden / deleted
  -- 文件信息
  raw_url       TEXT,
  hls_url       TEXT,
  thumbnail_url TEXT,
  duration      INTEGER,       -- 秒 (上限 120)
  file_size     BIGINT,        -- 字节 (上限 ~300MB)
  -- 元数据
  location_name VARCHAR(200),
  latitude      DECIMAL(10, 8),  -- 原始精度，仅后端存储
  longitude     DECIMAL(11, 8),  -- 原始精度，仅后端存储
  display_lat   DECIMAL(10, 2),  -- 脱敏精度 (±1km)，面向前端展示
  display_lng   DECIMAL(11, 2),  -- 脱敏精度 (±1km)，面向前端展示
  ...
  -- 审核
  moderation_status VARCHAR(20) DEFAULT 'auto_approved',  -- auto_approved / pending / approved / rejected / restricted
  moderated_by  UUID REFERENCES users(id),
  moderated_at  TIMESTAMPTZ,
  deleted_at    TIMESTAMPTZ,  -- 软删除时间，7天后 Worker 清理存储
  created_at    TIMESTAMPTZ DEFAULT NOW(),
  updated_at    TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE UNIQUE INDEX idx_bird_tags_unique ON bird_tags(video_id, bird_name);  -- 同一视频同一鸟种不可重复标注
```
...
CREATE TABLE notifications (
  ...
  archived_at TIMESTAMPTZ,                    -- 归档时间，90 天后由 Worker 自动归档
  created_at  TIMESTAMPTZ DEFAULT NOW()
);
...
```
uploaded ──→ queued ──→ processing ──→ ready ──→ deleted (软删除，7天后清理存储)
                            │
                            ▼
...
**审核状态（独立于技术状态）：**
```
auto_approved ──→ rejected (举报/规则触发后人工审核不通过)
auto_approved ──→ approved (人工复核通过)
auto_approved ──→ restricted (限流)
pending ──→ approved / rejected / restricted (人工审核)
```

> `status` 管理技术流水线状态，`moderation_status` 管理内容审核状态。
> 视频可见条件 = `status == ready` AND `moderation_status IN (auto_approved, approved)`
...
PATCH  /api/v1/notifications/read     # 批量标记已读
...
| # | 问题 | 决策 | 决策日期 |
|---|------|------|---------|
| OQ-1 | 首发版本是否引入第三方登录？ | **否**，降为 P2，首发后迭代 | 2026-03-08 |
| OQ-2 | 是否引入地理位置隐私模糊机制？ | **是**，精度降至 ±1km，原始精度仅后端存储 | 2026-03-08 |
| OQ-4 | 首发版本是否支持多语言？ | **否**，仅简体中文，架构预留 i18n（语言包独立） | 2026-03-08 |
| OQ-5 | 视频上传大小限制？ | **≤ 300MB**，基于 2min / H.265 / 2K 规格估算 | 2026-03-08 |
| OQ-7 | 云厂商选择？ | **待定**（AWS 或阿里云，Phase 2 staging 前决策），开发阶段本地 MinIO | 更新于 2026-03-09 |
...
| OQ-6 | 设备固件 SDK 由谁提供？是否已有现成方案？ | Phase 5 排期 | Phase 4 结束前 | 待确认，喂鸟器最后实现 |
...
> 每个 Milestone 达成后，PM 组织里程碑评审会，评审内容和产出：

| 评审项 | 说明 | 产出物 |
|-------|------|--------|
| 功能演示与验收 | 逐项核对该 Phase 的 Exit Criteria | 验收清单（通过/不通过） |
| 代码库质量 Review | 架构合理性、测试覆盖率、技术债务盘点 | 技术债务清单 |
| 文档完整性检查 | API 文档、设计文档是否与代码同步 | 文档更新 TODO |
| 下阶段 Scope 确认 | 根据实际进度调整下一 Phase 的范围 | 更新后的 PRD |
| 风险复盘 | 已暴露风险是否已缓解，是否有新风险 | 更新后的风险清单 |

**产出物：** 《Milestone Review Report》存放于 `doc/meeting/` 目录

### 18.5 文档目录规范

```
doc/
├── product/          # PRD、需求文档、决策记录
├── architecture/     # 架构设计、ADR、技术选型
├── api/              # API 文档 (OpenAPI)
├── design/           # UI/UX 设计规范、Design Token
├── testing/          # 测试计划、测试报告
├── operations/       # 运维手册、部署文档
└── meeting/          # 会议纪要、里程碑评审报告
```
